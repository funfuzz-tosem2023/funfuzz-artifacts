...
void write_bitmap(afl_state_t *afl) {
  ...
  afl->bitmap_changed = 0; // Reset this flag right after checking it.
...
// `inline`: Inline Function are those function whose definitions are small and
// be substituted at the place where its function call is happened. Function
// substitution is totally compiler choice.
// Seems `virgin_bits` are updated here. has_new_bits_unclassified() -> has_new_bits()
inline u8 has_new_bits(afl_state_t *afl, u8 *virgin_map) {
  ...
  // The value of `ret` may be influenced by `discover_word`.
  u8 ret = 0;
  // Traverse every bit in current bits and virgin bits.
  while (i--) {
...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  ...
  if (likely(fault == afl->crash_mode)) {
    ...
    // Do not save this test case (queue entry) if no new coverage found.
    if (likely(!new_bits)) {
    ...
    // todo: literally arranged in classes or categories. What is it for?
    classified = new_bits;

  save_to_queue: // May come back to here latter.
