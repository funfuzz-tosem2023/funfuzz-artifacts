 ...
 * seen in ret. */
// A polluting helper used in `afl-fuzz-bitmap.c#has_new_bits`. For every
// address for current bit (*current) and virgin bit (*virgin), this function
// update the bit value (*virgin &= ~*current) and determine the value of
// return as 0, 1, 2.
  ...
  if (*current & *virgin) {
    ...
    if (likely(*ret < 2)) {
      ...
      if ((cur[0] && vir[0] == 0xff) || (cur[1] && vir[1] == 0xff) ||
        ...
        // Update the value in `ret`'s address.
        *ret = 2;
      else
        *ret = 1;
    ...
    // No matter `ret` value is what, update current bit.
    *virgin &= ~*current;
...
// Skim: remove, deprecate. This function seems used for removing redundant
// bits. (I don't really understand what is `word` really refers to?)
inline u32 skim(const u64 *virgin, const u64 *current, const u64 *current_end) {
