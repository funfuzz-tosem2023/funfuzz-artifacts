...
/* Globals like LLVM_VERSION_MAJOR are read from system configurations */

#if LLVM_VERSION_MAJOR >= 11                        /* use new pass manager */
// RETURN_TYPE DECLARING_CLASS::METHOD_NAME(PARAM_LIST) { METHOD BODY }
...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  FUN_LOG("let's have some fun in afl-llvm-pass! *---------- \n");
  ...
  */
  ...
  // Determine whether needing to instrument ctx according to two envs
  // AFL_LLVM_CTX and AFL_LLVM_CALLER
  bool instrument_ctx = ctx_str || caller_str;
  FUN_LOG("ctx_str=%s\n", ctx_str);
  FUN_LOG("caller_str=%s\n", caller_str);
  FUN_LOG("instrument_ctx(bool)=%d\n", instrument_ctx);
  ...
  // Determine whether needing to instrument ctx according to AFL_CTX_K. This
  // env sets how long ctx we focus.
  if (ctx_k == 1) {

    ctx_k = 0; // `ctx_k == 1` equals call enabled and instrument_ctx
    instrument_ctx = true;
...
#endif
            MapPtrIdx);
        // M.getMDKindID: pass in a MD kind and get a non-zero ID
        // MDNode::get: pass in a LLVM CTX and return a MDTuple
