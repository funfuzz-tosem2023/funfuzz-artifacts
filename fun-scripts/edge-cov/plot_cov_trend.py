...
def group_cov_by_time(df: pd.DataFrame, draw_exec: bool) -> dict:
    ...
    for _bench in _benchmarks:
        ...
        # print(_inner_dict)
        _ave_dict = dict()
        for _fuzzer in _inner_dict:
            _ave_dict[_fuzzer] = _inner_dict[_fuzzer]['ave_edges'][-1]
        _df = pd.DataFrame(data=_ave_dict, columns=list(_ave_dict.keys()), index=[0])
        for _fuzzer in _inner_dict:
            _df[f'{_fuzzer}-diff'] = (_df[_fuzzer] - _df['aflpp']) / _df['aflpp']
        print(_df)
    return _dict
...
def plot_time_cov_in_range(
    ...
    # _ax.set(ylabel='Code Coverage')
    _ax.set(ylabel='Edge Coverage')
