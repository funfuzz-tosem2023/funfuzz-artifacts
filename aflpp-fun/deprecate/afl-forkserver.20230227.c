...
//// @FUN: debug
//void bits_ckpoint(char* ckp_id, afl_forkserver_t *fsrv) {
//  if (!getenv("FUN_DEBUG")) return ;
//  FUN_LOG(cCYA "checkpoint(%s) " cRST
//   "fsrv->call_cnts[0]=%d, fsrv->trace_bits[0]=%d, fsrv->trace_bits[1]=%d",
//    ckp_id, fsrv->call_cnts[0], fsrv->trace_bits[0], fsrv->trace_bits[1]);
//}
//static u32 __attribute__((hot))
//read_s32_timed_debug(s32 fd, s32 *buf, u32 timeout_ms,
//                     volatile u8 *stop_soon_p, afl_forkserver_t *fsrv) {
//
//  // debug
//  FUN_LOG(cCYA "read_s32_timed_debug" cRST ", "
//                 "fd %d, buf-p %p, timeout_ms %u, stop_soon_p %p.",
//            fd, buf, timeout_ms, stop_soon_p);
//
//  fd_set readfds;
//  FD_ZERO(&readfds);
//  FD_SET(fd, &readfds);
...
//
//  // checkpoint
//  bits_ckpoint("restart_select", fsrv);
//
//  sret = select(fd + 1, &readfds, NULL, NULL, &timeout);
//
//  // debug
//  FUN_LOG("fd+1 %d, &readfds %p, sret %d", fd + 1, &readfds, sret);
//
//  // Do read when select success?
//  if (likely(sret > 0)) {
//
//  restart_read:
//
//    // checkpoint
//    bits_ckpoint("restart_read", fsrv);
//
//    if (*stop_soon_p) {
//
...
afl_fsrv_run_target(afl_forkserver_t *fsrv, u32 timeout,
  ...
  // Read from fsrv process and update trace_bits.
  exec_ms = read_s32_timed(fsrv->fsrv_st_fd, &fsrv->child_status,
                           timeout, stop_soon_p);
  ...
  // Whether trace_bit can > 1? (the answer is yes)
  // Read non-zero values from trace_bits.
//  for (u32 i = 0 ; i < fsrv->map_size; ++i)
//    if (fsrv->trace_bits[i])
//      FUN_LOG("fsrv->trace_bits[%d]=%d", i, fsrv->trace_bits[i]);
//  exit(-1);
...
/* Execute target application, monitoring for timeouts. Return status
   information. The called program will update:
   (1) afl->fsrv->trace_bits, and
   (2) afl->fsrv->call_cnts.                    */
//#include <sys/shm.h>

//fsrv_run_result_t __attribute__((hot))
//afl_fun_fsrv_run_target(
//    afl_forkserver_t *fsrv, u32 timeout, volatile u8 *stop_soon_p) {
//
////  // debug
////  FUN_LOG("afl_fun_fsrv_run_target()!!!");
////  exit(-1);
//
//  s32 res;
//  u32 exec_ms;
//  u32 write_value = fsrv->last_run_timed_out;
//
//  // @FUN: This part is used on Ubuntu
//#ifdef __linux__
//  // Not use nyx_mode by default.
//  if (fsrv->nyx_mode) {
//
//    static uint32_t last_timeout_value = 0;
//
...
//     territory. */
//
//  // @FUN: This part is used on Ubuntu. Clean maps.
//#ifdef __linux__
//  if (!fsrv->nyx_mode) {
//
//    memset(fsrv->trace_bits, 0, fsrv->map_size);
...
//
//    // @FUN
//    memset(fsrv->call_cnts, 0, DEFAULT_FUN_SHM_SIZE);
//    MEM_BARRIER();
//
//  }
//
...
//  bits_ckpoint("5", fsrv);
//
//  // [*ATTENTION*] fsrv->trace_bits updated after this call!!!!!
//  // Read from fsrv process and update trace_bits.
//  // read_s32_timed(s32 fd, s32 *buf, u32 timeout_ms, volatile u8 *stop_soon_p)
//  exec_ms = read_s32_timed(fsrv->fsrv_st_fd, &fsrv->child_status,
//                           timeout, stop_soon_p);
////  exec_ms = read_s32_timed_debug(fsrv->fsrv_st_fd, &fsrv->child_status,
////                           timeout, stop_soon_p, fsrv);
//
//  // [*ATTENTION*] trace_bits has now been polluted, meaning that updating
//  // call_cnts and trace_bits are both done in `read_s32_timed()`.
//
//  // checkpoint
//  bits_ckpoint("6", fsrv);
//
...
//
//  // debug
////  FUN_LOG("fsrv->call_cnts[0] = %d", fsrv->call_cnts[0]);
////  exit(-1);
//  // Read non-zero values from trace_bits.
////  for (u32 i = 0 ; i < fsrv->map_size; ++i)
////    if (fsrv->trace_bits[i])
...
////  for (u32 i = 0 ; i < fsrv->map_size; ++i)
////    if (fsrv->trace_bits[i]) {
////      FUN_LOG("fsrv->trace_bits[%d]=%d, add=%p",
////                i, fsrv->trace_bits[i], &fsrv->trace_bits[i]);
////      FUN_LOG("fsrv->call_cnts[%d]=%d, add=%p",
////                i, fsrv->call_cnts[i], &fsrv->call_cnts[i]);
////    }
////  for (u32 i = 0 ; i < fsrv->call_cnts_len; ++i)
////    if (fsrv->call_cnts[i]) {
////      FUN_LOG("fsrv->call_cnts[%d]=%d", i, fsrv->call_cnts[i]);
////      if (fsrv->call_cnts[i] < 0) {
////        FATAL("Negative value at `i=%d`", i);
////      }
////    }
//
////  if (getenv(SHM_ENV_VAR))
////    FUN_LOG("afl_map_id=%s, trace_bits_p=%p",
////              getenv(SHM_ENV_VAR), fsrv->trace_bits);
////  if (getenv(FUN_SHM_ENV_VAR)) {
////    int cc_shm_id  = atoi(getenv(FUN_SHM_ENV_VAR));
////    u32 *fun_shm    = shmat(cc_shm_id, NULL, 0);
////    FUN_LOG("fun_map_id=%d, call_cnts_p=%p, fun_shm-addr=%p",
////              cc_shm_id, fsrv->call_cnts, fun_shm);
////    // What if write manually? -- the updates from fun_shm can be seen in call_cnts
////    fun_shm[10086] = 10086;
////    FUN_LOG(cCYA "fun_shm[10086] = 10086; !!!" cRST);
////    FUN_LOG(cCYA "[SHM_RES] " cRST "fun_shm[10086]=%d, call_cnts[10086]=%d",
////              fun_shm[10086], fsrv->call_cnts[10086]);
////  }
//
//  bits_ckpoint("afl_fun_fsrv_run_target-end", fsrv);
//
////  exit(-1);
//
//  /* success :) */
