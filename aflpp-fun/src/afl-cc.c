...
enum {
  ...
  // @FUN
  INSTRUMENT_PRE_FUN = 18,    // Extracting CG and CFGs.
  INSTRUMENT_FUN = 19,        // Computing significance score.

};

char instrument_mode_string[20][18] = {
...
static void edit_params(u32 argc, char **argv, char **envp) {
  ...
  if (compiler_mode == LLVM || compiler_mode == LTO) {
    ...
    if (lto_mode && !have_c) {
      ...
      // Choose working llvm pass. A lot of conditional compilations.
      ...
      // @FUN: Determine pass so.
      if (instrument_mode == INSTRUMENT_PRE_FUN) {
        FUN_LOG("Use pre-fun-pass.");
  #if LLVM_MAJOR >= 11                            /* use new pass manager */
          cc_params[cc_par_cnt++] = "-fexperimental-new-pass-manager";
          cc_params[cc_par_cnt++] = alloc_printf(
              "-fpass-plugin=%s/afl-llvm-prefun-pass.so", obj_path);
  #else
          cc_params[cc_par_cnt++] = "-Xclang";
          cc_params[cc_par_cnt++] = "-load";
          cc_params[cc_par_cnt++] = "-Xclang";
  ...
  #endif
      } else if (instrument_mode == INSTRUMENT_FUN) {
         FUN_LOG("Use fun-pass.");
  #if LLVM_MAJOR >= 11                            /* use new pass manager */
         cc_params[cc_par_cnt++] = "-fexperimental-new-pass-manager";
         cc_params[cc_par_cnt++] = alloc_printf(
             "-fpass-plugin=%s/SanitizerCoverageFun.so", obj_path);
  #else
         cc_params[cc_par_cnt++] = "-Xclang";
         cc_params[cc_par_cnt++] = "-load";
         cc_params[cc_par_cnt++] = "-Xclang";
      ...
      // Original aflpp instrumentation
  ...
  // Various sanitizers, asan, msan, ubsan, tsan...
  if (!asan_set) {
  ...
  // Code optimization, default is O3
  ...
  // Do not know... What do these builtins use for?
  ...
  // Add lots of code. Seems like var used by instrumented code at runtime
  cc_params[cc_par_cnt++] =
      "-D__AFL_FUZZ_INIT()="
  ...
  // Add different code under C/C++ mode.
  if (plusplus_mode) {
  ...
  if (compiler_mode != GCC_PLUGIN && compiler_mode != GCC) {
    ...
    // Maybe used for ignoring unused arguments?
    cc_params[cc_par_cnt++] = "-Wno-unused-command-line-argument";
  ...
  // Set cc_params according to identified modes (compiler_mode, inst_mode, lto_mode...).
  // cc_params are used to invoke the read CC executable.
  // Not a GCC or CLANG -> LTO LLVM GCC_PLUGIN
  if (compiler_mode != GCC && compiler_mode != CLANG) {
...
int main(int argc, char **argv, char **envp) {
  ...
  int   i;                  // ptr seems a conventional temporary var in c.
  char *callname = argv[0], *ptr = NULL;

  // callname: the path to call afl-cc, can be afl-cc, afl-gcc, afl-clang-fast etc.
  // This is essentially the first parameter of the command interface (argv[0]).
  if (debug) FUN_LOG("Have fun in afl-cc! callname=%s, argc=%d", callname, argc);
  ...
  // strrchr: find the last occurrence of a substring in the given string.
  // This step shorten the callname from full file path to simple filename.
  if ((ptr = strrchr(callname, '/')) != NULL) callname = ptr + 1;
  argvnull = (u8 *)argv[0];
  check_environment_vars(envp);

  // Do some finds to confirm available plugins
  ...
  // Determine the actual working mode.
#if (LLVM_MAJOR >= 3)
  ...
  if (debug) FUN_LOG("Before checking argv");

  // argc: arguments count. The number of arguments
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (strncmp(argv[i], "--afl", 5) == 0) {
      ...
      ptr = argv[i];
      ptr += 5;
      while (*ptr == '-') // Skip opt name and find the first argument?
        ptr++;

      // strncasecmp: ignore case, compare the first n char bt two strings str1 and str2
      if (strncasecmp(ptr, "LTO", 3) == 0) {
  ...
  } // End of argv checking
  if (debug) FUN_LOG("Finish checking argv");

  // Determine instrumentation mode. Not like compiler_mode, ins_mode can be
  // multiplied trh |
  ...
  if (getenv("USE_TRACE_PC") || getenv("AFL_USE_TRACE_PC") ||
    ...
    if (instrument_mode == 0) // Why not INSTRUMENT_DEFAULT?
      instrument_mode = INSTRUMENT_PCGUARD;
    else if (instrument_mode != INSTRUMENT_PCGUARD)
  ...
  if (getenv("AFL_LLVM_INSTRUMENT")) {
    ...
    if (debug) FUN_LOG("Parsing AFL_LLVM_INSTRUMENT...");

    u8 *ptr2 = strtok(getenv("AFL_LLVM_INSTRUMENT"), ":,;");

    if (debug) FUN_LOG("Traverse instrument options...");
    while (ptr2) {

      // @FUN
      if (strncasecmp(ptr2, "prefun", strlen("prefun")) == 0 ||
          strncasecmp(ptr2, "pre-fun", strlen("pre-fun")) == 0) {
        if (debug)
          FUN_LOG("Previous inst_mode=%d, compiler_mode=%d.",
             instrument_mode, compiler_mode);
        instrument_mode = INSTRUMENT_PRE_FUN;
        compiler_mode = LLVM;
      }

      if (strncasecmp(ptr2, "fun", strlen("fun")) == 0 ||
          strncasecmp(ptr2, "funsancov", strlen("funsancov")) == 0 ||
          strncasecmp(ptr2, "fun-sancov", strlen("fun-sancov")) == 0) {
        if (debug)
          FUN_LOG("Previous inst_mode=%d, compiler_mode=%d.",
             instrument_mode, compiler_mode);
        instrument_mode = INSTRUMENT_FUN;
        compiler_mode = LLVM;
      }

      // "afl" and "classic" both refer to classic instrumentation.
      if (strncasecmp(ptr2, "afl", strlen("afl")) == 0 ||
          strncasecmp(ptr2, "classic", strlen("classic")) == 0) {

        // LTO = CLASSIC + setting lto_mode.
        // Basically, INSTRUMENT_CLASSIC == INSTRUMENT_AFL
  ...
  if (debug) FUN_LOG("After parsing AFL_LLVM_INSTRUMENT.");

  // Check instrumentation feasibility.
...
#undef NATIVE_MSG
  ...
  // >= GCC_PLUGIN: GCC_PLUGIN, GCC, CLANG
  ...
  if (passthrough) {
    ...
    // process image specified by file. The new image is constructed from a
    // regular, executable file called the new process image file. No return
    // is made because the calling process image is replaced by the new process
    // image.
