...
#  error AFLGO has not supported trace-pc-guard yet
  ...
  cc_params[cc_par_cnt++] = "-Xclang";
  cc_params[cc_par_cnt++] = alloc_printf("%s/fundump-pass.so", obj_path);
#endif /* ^USE_TRACE_PC */
...
#ifdef USE_TRACE_PC
    SAYF(cCYA "fundump-compiler (yeah!) [tpcg] " cBRI VERSION  cRST "\n");
#else
    SAYF(cCYA "fundump-compiler (yeah!) " cBRI VERSION  cRST "\n");
#endif /* ^USE_TRACE_PC */
