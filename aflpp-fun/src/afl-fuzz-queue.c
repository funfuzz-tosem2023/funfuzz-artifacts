...
double compute_est_coe(afl_state_t *afl) {
...
inline double compute_scale_fun(double val, double est_val, double rho) {

  if (est_val == 0) return 0.2;

  double scale_fun, est_err;
  ...
  // Let scale_fun range in [0.2, 2.0]
...
double adjust_fun_factor(
  ...
  scale_fun = compute_scale_fun(norm_sig, est_norm_sig, fun->rho);
