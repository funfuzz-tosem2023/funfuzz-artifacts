...
#include "debug.h"
static inline void debug_for_fun(char *mes) {
  if (!!getenv("FUN_DEBUG")) FUN_LOG("%s", mes);
}
...
static void send_forkserver_error(int error) {
  ...
  // debug
  debug_for_fun("send_forkserver_error");
...
static void __afl_map_shm_fuzz() {
  ...
  // debug
  debug_for_fun("__afl_map_shm_fuzz");
  ...
  if (id_str) {
    ...
    if (__afl_area_ptr && __afl_area_ptr != __afl_area_initial &&
      ...
      if (__afl_map_addr) {
        ...
        // Debug
        debug_for_fun("(__afl_map_addr)-else");
    ...
    // Debug
    debug_for_fun("USEMMAP does not work!");
    ...
    if ((void *)__afl_map_addr == NULL)
      debug_for_fun("(void *)__afl_map_addr == NULL!");
    else
      debug_for_fun("(void *)__afl_map_addr != NULL!");
    ...
    // Debug
    debug_for_fun("Get into id_str, else if (1)");
    ...
    // Debug
    debug_for_fun("Get into id_str, else if (2)");
...
// SHM_ENV_VAR
// This method is not necessary if `__sanitizer_cov_trace_pc_guard_init`
// is not called.
...
static void __afl_start_forkserver(void) {
  ...
  // debug
  debug_for_fun("__afl_start_forkserver");
...
int __afl_persistent_loop(unsigned int max_cnt) {
  ...
  // debug
  debug_for_fun("__afl_persistent_loop");
...
void __afl_manual_init(void) {
  ...
  // debug
  debug_for_fun("__afl_manual_init");
...
__attribute__((constructor(EARLY_FS_PRIO))) void __early_forkserver(void) {
  ...
  // debug
  debug_for_fun("__early_forkserver");
...
__attribute__((constructor(CTOR_PRIO))) void __afl_auto_early(void) {
  ...
  // debug
  debug_for_fun("__afl_auto_early");
...
__attribute__((constructor(1))) void __afl_auto_second(void) {
  ...
  // debug
  debug_for_fun("__afl_auto_second");
...
__attribute__((constructor(0))) void __afl_auto_first(void) {
  ...
  // debug
  debug_for_fun("__afl_auto_first");
...
// A runtime function defined in "SanitizerCoveragePCGuard.so.cc", known as
// `SanCovTracePCGuardName[] = "__sanitizer_cov_trace_pc_guard"`.
void __sanitizer_cov_trace_pc_guard(uint32_t *guard) {

  // Debug
  debug_for_fun("__sanitizer_cov_trace_pc_guard");
...
void __sanitizer_cov_trace_pc_guard_init(uint32_t *start, uint32_t *stop) {
  ...
  // Debug
  debug_for_fun("__sanitizer_cov_trace_pc_guard_init");
  ...
  *(start++) = ++__afl_final_loc;
  ...
  // Unmap then map if this condition is satisfied. The semantic of this
  // condition: shm has already initialized (__afl_map_shm()) called, but
  // the needed memory (obtained through dynamic analysis) exceeds the
  // given map size. As a result, the shm should be reallocated. This
  // operation is presented as invoking again `__afl_map_shm()` right after
  // freeing already allocated shm through `__afl_unmap_shm()`.
...
// Write after __afl_auto_early
#define FUN_CTOR_PRIO   (CTOR_PRIO + 1)

#define FUN_FUNC_NUM    (1U << 13)
#define FUN_AREA_SIZE   (FUN_FUNC_NUM * FUN_FUNC_NUM)

static u64  __fun_area_init[FUN_AREA_SIZE];
static u64  *__fun_area_ptr_dummy = __fun_area_init;

// Runtime ptr
u64 *__fun_area_ptr = __fun_area_init;

// Already initialized flags
u32 __fun_already_initialized_shm;
u32 __fun_already_initialized;

// Lots of variables
u64 __fun_map_addr;

__attribute__((constructor(FUN_CTOR_PRIO)))
void __fun_auto_early(void) {

  debug_for_fun("__fun_auto_early()");

  if (!getenv(FUN_TEMP_DIR)) return ;
...
// @FUN: pass shm_id and attach __fun_area_ptr to the shm.
static void __fun_map_shm(void) {

  debug_for_fun("__fun_map_shm() works! ");

  if (__fun_already_initialized_shm) return;
  __fun_already_initialized_shm = 1;

  // if we are not running in afl ensure the map exists
  if (!__fun_area_ptr) { __fun_area_ptr = __fun_area_ptr_dummy; }

  u8 *id_str = getenv(FUN_CC_SHM_ENV_VAR);

  // Read hot cc_shm_id from env
  if (id_str) {

    if (__fun_area_ptr
        && (__fun_area_ptr != __fun_area_init)
        && (__fun_area_ptr != __fun_area_ptr_dummy)) {

      // Delete lots of code used in afl-trace_bits construction.
      __fun_area_ptr = __fun_area_ptr_dummy;
    ...
    // Attach __fun_area_ptr to hot cc_shm
    __fun_area_ptr = (u64 *) shmat(shm_id, NULL, 0);

    /* Whooooops. */

    if (!__fun_area_ptr || __fun_area_ptr == (void *)-1) {

      // Exit with abnormal exitcode.
      fprintf(stderr, "shmat() failed for __fun_area_ptr!\n");
      abort();
...
static void __fun_unmap_shm(void) {
  ...
  char *id_str = getenv(FUN_CC_SHM_ENV_VAR);
  ...
  if (id_str) {
    ...
    // Reverse function for `shmat()`.
    // int shmdt(const void *shmaddr);
    shmdt((void *)__fun_area_ptr);

  } else if ((!__fun_area_ptr || __fun_area_ptr == __fun_area_init)) {

    // Exit with abnormal exitcode.
    FATAL("shmdt() failed for __fun_area_ptr!\n");
  ...
  // Reset shm flag.
  __fun_already_initialized_shm = 0;
