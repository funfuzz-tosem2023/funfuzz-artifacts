...
// (Guess) PSO = Particle Swarm Optimization
...
//  FUN_LOG("Come into fuzz_one()\n");

  // Keep the return value of the real fuzzing utility, e.g., pilot_fuzzing(),
  // core_fuzzing(), etc.
...
// @FUN: Modified from fuzz_one_original()
// ReturnValue: returns 0 if fuzzed successfully, 1 if skipped or bailed out.
u8 fuzz_one_fun(afl_state_t *afl) {

  // Debug
//  FUN_LOG("Use fuzz_one_fun()!\n");
//  FUN_LOG("default afl->skip_deterministic=%d!\n",
//       afl->skip_deterministic);
//  FUN_LOG("afl->power_name=%s!\n", afl->power_name);
//  FUN_LOG("afl->use_banner=%s!\n", afl->use_banner);
//  FUN_LOG("afl->sync_id=%s!\n", afl->sync_id);
//  exit(-1);
  ...
  // Read in bits of the current test case. `orig_in` to preserve original bits,
  // `in_buf` used for mutation.
  orig_in = in_buf = queue_testcase_get(afl, afl->queue_cur);
  len = afl->queue_cur->len;
  ...
  // To help test cases lose weight.
  if (unlikely(!afl->non_instrumented_mode && !afl->queue_cur->trim_done &&
               !afl->disable_trim)) {
  ...
  if (likely(afl->queue_cur->passed_det) || likely(afl->skip_deterministic) ||
    ...
    // Debug
//    FUN_LOG("Skip deterministic stage, jump to custom_mutator_stage");
//    exit(-1);
...
//    exit(-1);
  ...
  // Debug
//  FUN_LOG("Go into deterministic stage");
//  exit(-1);

  doing_det = 1;

  // These deterministic mutators won't be used in most cases.
  // Deterministic mutators -- start
  /*********************************************
   * SIMPLE BITFLIP (+dictionary construction) *
   *********************************************/
...
//  exit(-1);
  ...
  // (Guess) So the above code lines are used for registering mutators?
  if (afl->custom_mutators_count) {
