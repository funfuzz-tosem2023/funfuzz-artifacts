...
int main(int argc, char **argv_orig, char **envp) {
  ...
  } // Parsing command line options. -- end
  ...
  afl->fsrv.kill_signal =
      parse_afl_kill_signal_env(
          afl->afl_env.afl_kill_signal, SIGKILL);

  setup_signal_handlers();
  check_asan_opts(afl);

  // Set name of power scheduling according to the schedule mode (an enum).
  ...
  // Validate input (initial seeds) and output directory
  ...
  // `afl_env_vars_t` is a type in type.
  ...
  switch (afl->schedule) {
    ...
    // @FUN
    case FUN:
      OKF("Using function-significance-aware power schedule (FUN)");
      break;
    default:
  ...
  get_core_count(afl);  // Prepare CPU

  atexit(at_exit);      // Prepare exiting (?)

  setup_dirs_fds(afl);  // Prepare output directory

  // @FUN: configure fun mode

  fun_globals_t *fun  = &afl->fun;
  u8 *fun_tmpdir      = getenv(FUN_TEMP_DIR);

  if (fun_tmpdir) {

    // Initialize @FUN mode
    fun_init(afl, fun_tmpdir);
  ...
  // (Guess) Node refers to the "node that executes fuzzing".
  ...
  // Read in all initial seeds.
  read_testcases(afl, NULL);
  // read_foreign_testcases(afl, 1); for the moment don't do this
  OKF("Loaded a total of %u seeds.", afl->queued_items);

  // (Guess) some optimizations. Build hard links for initial seeds.
  pivot_inputs(afl);

  if (!afl->timeout_given) { find_timeout(afl); }  // only for resumes!

  // Tmp file to store current test input.
  ...
  // (What is) Shared memory fuzzing.
  if (afl->shmem_testcase_mode) { setup_testcase_shmem(afl); }

  // Set start time.
  afl->start_time = get_cur_time();

  // Rewrite argv according to qemu_mode.
  if (afl->fsrv.qemu_mode) {
  ...
  // Finally set the real argv used for fuzzing.
  afl->argv = use_argv;
  ...
  if (afl->fsrv.out_file && afl->fsrv.use_shmem_fuzz) {
    ...
    if (!afl->unicorn_mode && !afl->fsrv.use_stdin) {
      ...
      // It seems that some fuzz targets receive inputs through shmem.
      WARNF(
  ...
  // "-B" is an implicit option which is used for reproduction.
  if (afl->in_bitmap) {
  ...
  // May not calibrate if configured.
  if (likely(!afl->afl_env.afl_no_startup_calibration)) {

    // Validate initial seeds.
    perform_dry_run(afl);

    // @FUN

    if (fun->on && !fun->fs_static) {

      FUN_LOG("Initialize fs_shm with dry_run data, wait for a moment...");

      // Clear the hot fs_shm
      memset(fun->fs_shm[fun->fs_idx_hot], 0, fun->fs_shm_size);

      // In dynamic_fs mode, initialize fs_shm with data obtained in dry_run.
      // Here we wait DCC component to finish (wait=1).
      dcc_update_fs_vals(afl, 1);

    }

    // Compute sig_score for test cases that have already been in the queue.
    compute_sig_score_for_queue(afl);
    ...
    // @FUN
    if (fun->on && !fun->fs_static)
      FATAL("Must perform_dry_run() under dynamic @FUN mode!!!");
  ...
  // Flag for fuzzing started
  show_init_stats(afl);

  // What is `vanilla afl seed selection`? The `find_start_position`?
  ...
  if(fun->on) {
    ...
    // Record loop start time in dynamic mode
    if (!fun->fs_static) fun->last_fs_update_time = time(NULL);
  ...
  while (likely(!afl->stop_soon)) {
    ...
    // @FUN: update fs periodically.
    if (fun->on && !fun->fs_static) {

      // Ensure the DCC component alive
      ck_dcc_status(afl, "main()");

      // Check and update FS values periodically.

      cur_time_tmp = time(NULL);

      if (unlikely(cur_time_tmp - fun->last_fs_update_time
                   >= fun->fs_update_period)) {

        // Update period exceeded, refresh last_fs_update_time
        fun->last_fs_update_time = cur_time_tmp;

        // Ask DCC to compute new FS values for us. Let DCC work in parallel
        // with the main fuzzing loop this time (wait=0).
        dcc_update_fs_vals(afl, 0);
    ...
    if (unlikely((!afl->old_seed_selection &&
      ...
      // It seems `cycle_schedules` means `using different schedules in cycle`.
      if (afl->cycle_schedules) {
        ...
        switch (afl->schedule) {
          ...
          // Update schedule to the next one.
          case EXPLORE:
            afl->schedule = EXPLOIT;
            break;
          case EXPLOIT:
      ...
      // The number of queue entries in the last iteration.
      prev_queued = afl->queued_items;

    }

    // Record number of runs in each cycle.
    ...
    do {
      ...
      skipped_fuzz = fuzz_one(afl); // Dispatch to real fuzz utility
  ...
  } // Main fuzzing loop -- end

// Lots of postprocessing
stop_fuzzing:
  ...
  // @FUN: before destroy queue, record fs information
  if (fun->on) record_sigdata_for_queue(afl);
  ...
  // @FUN: Free fun globals as well as SHMs.
  if (fun->on) fun_deinit(afl);
