...
mkdir -p "$IN_DIR"
cp "$SEEDS"/others/pdf/*.pdf "$IN_DIR"

# Global settings
export AFL_FUN_TEMP="$TARGET_DIR/funtmp"
export FUN_DCC="$FUNFUZZ/fun/fs/dyn_compute_fs.py"
...
for idx in $(seq "$START_IDX" "$END_IDX"); do
  ...
  # Run fuzzing.
  "$FUZZER" -V "$DURATION" -i "$IN_DIR" -o "$OUT_DIR" "$TARGET_PATH" draw @@ -o "$TARGET_DIR/out"
