...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  // @FUN: only conduct constant-based score adjustment under non-edgeonly mode.
  if (unlikely(afl->schedule != EDGEONLY)) {
  ...
  switch (afl->schedule) {
    ...
    // @FUN: Edgeonly mode. Scale perf_score w.r.t. (input's bitmap / ave_bitmap)
    case EDGEONLY:

      perf_score *= (q->bitmap_size / avg_bitmap_size);
      break;
